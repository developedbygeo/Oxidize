...
use crate::types::{
    AudioExtractOptions, VideoCompressOptions, VideoConvertOptions, VideoInfo, VideoQualityMode,
    VideoResizeMode, VideoResizeOptions, VideoResult,
};
...
pub async fn extract_audio_batch(
    ...
    for path in input_paths {
        ...
        let result = extract_audio(app.clone(), path.clone(), options.clone())
            .await
            .unwrap_or_else(|e| VideoResult {
                success: false,
                input_path: path,
